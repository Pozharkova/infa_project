...
def init_surface(screen, surface, max_x, max_y):
    ...
    for i in range(len(surface.points)):
        ...
        if i < len(surface.points) - 1:
            ...
            lines_id.append(screen.create_line(x, y, scale_x(surface.points[i + 1].x, max_x),
                                               scale_y(surface.points[i + 1].y, max_y), fill=color, activefill='red',
                                               width=2))
...
start_btn = tk.Button(mod_frame, text="Start", command=start_click)
start_btn.pack(side='left')
pause_btn = tk.Button(mod_frame, text="Pause", command=pause_click)
pause_btn.pack(side='left')
stop_btn = tk.Button(mod_frame, text="Stop", command=stop_click)
...
def new_sim(event=''):
    ...
    targets.append(bod.Target(5, 2.5, 0.2))
    targets.append(bod.Target(2, 3, 0.5))
